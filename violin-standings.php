<?php
    ...
    if (isset($_POST["cat1"])){
        $sql = "
            ...
            INNER JOIN pieces ON contestants.ID=pieces.ContestantID) 
            INNER JOIN scores ON contestants.ID=scores.ContestantID AND pieces.PieceID=SCORES.PieceID)
            WHERE contestants.Instrument='violin' AND pieces.Category='option1'
    ...
    if (isset($_POST["cat2"])){
        $sql = "
            ...
            INNER JOIN pieces ON contestants.ID=pieces.ContestantID) 
            INNER JOIN scores ON contestants.ID=scores.ContestantID AND pieces.PieceID=SCORES.PieceID)
            WHERE contestants.Instrument='violin' AND pieces.Category='option2'
            ORDER BY scores.OverallScore DESC;
        ";
    }
    ...
    if (isset($_POST["cat3"])){
        $sql = "
            ...
            INNER JOIN pieces ON contestants.ID=pieces.ContestantID) 
            INNER JOIN scores ON contestants.ID=scores.ContestantID AND pieces.PieceID=SCORES.PieceID)
            WHERE contestants.Instrument='violin' AND pieces.Category='option3'
            ORDER BY scores.OverallScore DESC;
        ";
    }
...
<body>
    ...
    <div class="container section">
        ...
        <a href="piano-standings.php" class="btn">Piano Results</a>

        <h4 class="center"><a href="violin-standings.php">Overall</a> violin results</h4>
        <h6 class="center">Select one to see results on that category.</h6>

        <div class="section center">
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                <div class="section">
                    <input type="submit" value="category I" name="cat1" class="btn z-depth-0">
                    <input type="submit" value="category II" name="cat2" class="btn z-depth-0">
                    <input type="submit" value="category III" name="cat3" class="btn z-depth-0">
                </div>
            </form>
        </div>
